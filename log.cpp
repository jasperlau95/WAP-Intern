...
void Log::UserLog(string loginname, string username, string operation, int status)			// Add new information to UserLog.txt
{
	...
	fout << "[ " << time.wYear << "-" << time.wMonth << "-" << time.wDay << " " << time.wHour << ":" << time.wMinute << ":" << time.wSecond << " ]";			// Output system tiime
	if (operation == "add") 
...
{
	ofstream fout;
	fout.open(book_log_address, ios::app);  						// Open and append to BookLog.txt
	if (!fout) {												// If can't open
...
{
	ofstream fout;
	fout.open(cash_log_address, ios::app); // Open and append to BookLog.txt
	if (!fout) // If can't open	
	{					
